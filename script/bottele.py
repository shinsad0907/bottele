...
# ── Chi phí ───────────────────────────────────────────────────────────────────
COST_IMAGE = int(os.getenv("COST_IMAGE", "60"))   # ← tăng lên 60xu
...
def kb_main(u: dict, lang: str = DEFAULT_LANG) -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup([
        ...
        # ── Mời bạn (external_link tạm ẩn) ───────────────────────────────────
        [InlineKeyboardButton(t("btn_referral", lang), callback_data="referral")],
        # ── Thống kê & hướng dẫn ─────────────────────────────────────────────
        [InlineKeyboardButton(t("btn_stats", lang), callback_data="stats")],
        # ── Ngôn ngữ (hardcode để chắc chắn hiện) ────────────────────────────
        [InlineKeyboardButton("🌐 Ngôn Ngữ / Language", callback_data="language")],
        # ── Điểm danh ────────────────────────────────────────────────────────
        [rc_btn],
    ])
...
def kb_after_image(lang: str = DEFAULT_LANG) -> InlineKeyboardMarkup:
    """Sau khi tạo ảnh xong – không có nút video vì tính năng tạm ẩn."""
    return InlineKeyboardMarkup([
        [
...
def kb_prompt_selector(lang: str = DEFAULT_LANG) -> InlineKeyboardMarkup:
    """Keyboard chọn preset prompt từ PROMPT_PRESETS."""
    rows = []
    for label, val in PROMPT_PRESETS:
        cb = "preset___custom__" if val == "__custom__" else f"preset_{val}"
...
async def btn(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ── External link (tạm ẩn – giữ handler phòng link cũ) ───────────────────
    if d == "external_link":
        await q.answer("⏳ Tính năng tạm thời bảo trì.", show_alert=True)
        return
    ...
    # ── Create video (tạm ẩn – giữ handler phòng link cũ) ────────────────────
    if d == "create_video":
        await q.answer("⏳ Tính năng video đang bảo trì.", show_alert=True)
        return
    ...
    if d.startswith("preset_"):
        # callback_data: "preset___custom__" hoặc "preset_<prompt text>"
        value = d[len("preset_"):]
...
async def on_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if state == "awaiting_image_photo":
        if not msg.photo:
            await msg.reply_text(
                t("img_no_photo", lang), parse_mode=ParseMode.MARKDOWN_V2
            )
            return

        file_id = msg.photo[-1].file_id
        context.user_data["pending_photo_file_id"] = file_id
        context.user_data["state"] = "awaiting_prompt"   # chờ chọn preset
    ...
    # ── Awaiting video photo (tạm ẩn – vẫn giữ handler) ─────────────────────
    if state == "awaiting_video_photo":
        await msg.reply_text(
            "⏳ Tính năng video đang bảo trì\\.", parse_mode=ParseMode.MARKDOWN_V2
        )
...
# ══════════════════════════════════════════════════════════════════════════════
# Video generation (giữ nguyên code nhưng không gọi từ UI)
# ══════════════════════════════════════════════════════════════════════════════
...
async def cmd_addcoins(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        return
    ok, new_bal = admin_add_coins(username, amount)
    if not ok:
        await update.message.reply_text(
            t("admin_user_notfound", lang, user=username), parse_mode=ParseMode.MARKDOWN_V2
        )
        return
    await update.message.reply_text(
...
async def cmd_setpackage(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if pkg not in ("free", "vip", "vip_pro"):
        ...
        return
    ok = admin_set_package(username, pkg)
    if not ok:
        await update.message.reply_text(
            t("admin_user_notfound", lang, user=username), parse_mode=ParseMode.MARKDOWN_V2
        )
