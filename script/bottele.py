...
from database import (
    get_or_create_user, get_user_db, spend_coins, add_coins,
    increment_image_count, increment_video_count, increment_proxy_count,
    set_status, remove_from_clothesAI,
)
from queue_manager import enqueue, release, is_free_user
import payment_handler as payment_handler
...
ADMIN_IDS = [123456789]   # ← Thay bằng Telegram ID của admin
...
async def send_join_prompt(reply_fn):
    text = (
        ...
        "1️⃣ Bấm nút bên dưới → Join channel\n"
        "2️⃣ Quay lại bấm /start"
    )
...
# ══════════════════════════════════════════════
#  FIREBASE / IMAGE / VIDEO  (giữ nguyên)
# ══════════════════════════════════════════════
...
# ── RAM session DB (không lưu coins ở đây, dùng Supabase) ────────────────────
keys_db     = {}
sessions_db = {}
...
def coin_bar(coins, max_coins=200):
...
def splash_final(name, coins, total_images, total_bypassed, total_videos=0, package="free"):
    ...
    pkg_icon = {"free": "🆓", "VIP": "⭐", "VIP PRO": "💎"}.get(package, "🆓")
    return (
        "```\n"
        "╔══════════════════════════════════════╗\n"
        "║ 👗✨━━━━━━━━━━━━━━━━━━━━━━━━━━✨👗 ║\n"
        ...
        f"│  {badge}\n"
        f"│  {pkg_icon} Gói: `{esc(package)}`\n"
        f"│  💰 Xu: `{coins}` {bar}\n"
        f"│  🎨 Ảnh đã tạo:   `{total_images}`\n"
        f"│  🎬 Video đã tạo: `{total_videos}`\n"
        f"│  🔗 Lượt kiếm xu: `{total_bypassed}`\n"
        f"└────────────────────────────────┘\n\n"
        f"⚡ Tạo ảnh: `{COST_IMAGE} xu` \\| 🎬 Video: `{COST_VIDEO} xu`\n"
        f"🎁 Kiếm xu: `+{BYPASS_REWARD} xu` / link\n\n"
...
def kb_main(coins, package="free"):
    ...
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton("💳 Mua Xu / VIP",            callback_data="payment_menu")],
        [InlineKeyboardButton("📊 Thống Kê",                callback_data="stats"),
         InlineKeyboardButton("📖 Hướng Dẫn",               callback_data="help")],
        [InlineKeyboardButton("👗✨━━━━━━━━━━━━━━━━━━✨👗", callback_data="noop")],
        [InlineKeyboardButton(f"{badge} {pkg_icon} • 💰 {coins} xu", callback_data="balance")],
    ])
...
def msg_balance(full_name, uid, coins, total_images, total_bypassed, total_videos=0, package="free"):
    ...
    return (
        ...
        f"│  💵 Tổng nhận:    `{earned + 100} xu`\n"
        f"└──────────────────────────────┘\n\n"
        f"💳 Muốn thêm xu? Bấm *Mua Xu / VIP*"
    )
...
def msg_bypass(link):
    return (
        ...
        f"📋 *Hướng dẫn:*\n"
        f"┌──────────────────────────────┐\n"
        f"│  1️⃣  Bấm link bên dưới        │\n"
        f"│  2️⃣  Hoàn thành trên web       │\n"
        f"│  3️⃣  Sao chép mã KEY           │\n"
        f"│  4️⃣  Bấm \"Nhập Key\" → dán vào │\n"
        f"└──────────────────────────────┘\n\n"
        f"⚠️ *Mỗi key chỉ dùng 1 lần*"
    )

def msg_queue_waiting(position: int, wait_seconds: int) -> str:
    return (
        "```\n"
        "╔══════════════════════════════════════╗\n"
        "║  ⏳  CLOTHESBOT  ·  HÀNG CHỜ  ⏳    ║\n"
        ...
        "╚══════════════════════════════════════╝\n"
        "```\n\n"
        f"🔢 Vị trí của bạn: *#{position}*\n"
        f"⏱ Thời gian chờ ước tính: `~{wait_seconds}s`\n\n"
        "💡 Để bỏ qua hàng chờ, nâng cấp *VIP* hoặc *VIP PRO*\\!\n"
        "💳 Bấm /start → *Mua Xu / VIP*"
    )
...
def msg_help():
    return (
        ...
        "1\\. Bấm `🎨 ✨ Tạo Ảnh`\n"
        "2\\. Gửi ảnh gốc\n"
        "3\\. Nhập mô tả bằng tiếng Anh\n"
        "4\\. Đợi xử lý \\(~20\\-40 giây\\)\n"
        ...
        "4\\. Đợi render \\(~2\\-5 phút\\)\n"
        f"💰 Chi phí: `{COST_VIDEO} xu` / lần\n\n"
        "👗✨━━━━━━━━━━━━━━━━━━━━━✨👗\n"
        "💳 *MUA XU / VIP*\n"
        "👗✨━━━━━━━━━━━━━━━━━━━━━✨👗\n"
        "→ Bấm `💳 Mua Xu / VIP` trong menu\n"
        "→ Chuyển khoản theo hướng dẫn\n"
        "→ Xu được cộng sau khi duyệt\n\n"
        "⭐ VIP: `69k/tháng` — 700 xu/ngày \\+ bỏ qua hàng chờ\n"
        "💎 VIP PRO: `149k/tháng` — 3000 xu/ngày \\+ bỏ qua hàng chờ"
    )

# ══════════════════════════════════════════════
#  VIDEO + IMAGE API (giữ nguyên từ code gốc)
# ══════════════════════════════════════════════
...
def _pika_signup(sess, email, password, username):
    ...
    sess.post("https://pika.art/signup",
        headers={"accept":"text/x-component","next-action":ah,
                 "next-router-state-tree":("%5B%22%22%2C%7B%22children%22%3A%5B%22(entry)%22%2C%7B%22children%22%3A%5B"
...
def _pika_generate_job(access_token, user_id, image_bytes, image_filename, prompt="gentle movement", model="2.5", duration=5, resolution="480p"):
    ...
    resp = requests.post("https://api.pika.art/generate/v2",
        ...
        files={"resolution":(None,resolution),"promptText":(None,prompt),"image":(image_filename,image_bytes,mime),
               "duration":(None,str(duration)),"model":(None,model),"contentType":(None,"i2v"),
               "options":(None,options),"creditCost":(None,"12"),"userId":(None,user_id)},
        timeout=60)
    data = resp.json()
    ...
    while time.time() < deadline:
        try:
            resp = requests.post("https://pika.art/library",
                headers={"accept":"text/x-component","accept-language":"vi-VN,vi;q=0.9,en;q=0.5",
                         "content-type":"text/plain;charset=UTF-8","next-action":lib_hash,
                         "next-router-state-tree":("%5B%22%22%2C%7B%22children%22%3A%5B%22(dashboard)%22%2C%7B%22children"
                         ...
                         "origin":"https://pika.art","referer":"https://pika.art/library",
                         "user-agent":PIKA_UA,"cookie":cookie_str},
        ...
        time.sleep(interval)
    raise Exception(f"Quá thời gian {timeout}s")
...
def pika_create_account_and_generate(image_bytes, filename, prompt="gentle movement", log_cb=None):
    ...
    push(f"  ◈ Prompt : {prompt[:30]}", 0); push(f"  ◈ Output : 🎬 480p | ⏱ 5 giây", 0)
    push(f"  ┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄┄", 0)
    ...
    sess = requests.Session(); sess.headers.update({"user-agent":PIKA_UA})
    ...
    vr = requests.Session(); vr.headers.update({"user-agent":PIKA_UA})
    ...
    job_id = _pika_generate_job(lr["access_token"], lr["user_id"], image_bytes, filename, prompt=prompt)
...
def create_account():
    email = random_email()
    r = requests.post('https://identitytoolkit.googleapis.com/v1/accounts:signUp',
        ...
        timeout=15).json()
    if 'idToken' not in r: raise Exception(f"Firebase error: {r.get('error',{}).get('message',str(r))}")
    r2 = requests.post('https://sv.aivideo123.site/api/user/init_data', headers=API_HDR,
        json={'token':r['idToken'],'code':'-1','login_type':0,'current_uid':''}, timeout=15).json()
    if r2.get('code') != 1: raise Exception(f"init_data error: {r2}")
...
def generate_image(image_bytes, filename, prompt, log_cb=None):
    ...
    up = requests.post(s3_url, data=fields, files={"file":(filename,image_bytes,"image/jpeg")}, timeout=30)
    ...
    inf = requests.post("https://sv.aivideo123.site/api/item/inference2",
        headers=headers, json={"s3_path":s3_key,"mask_path":"","prompt":prompt,"ai_model_type":3}, timeout=15).json()
...
#  ANIMATED SPLASH
# ══════════════════════════════════════════════
async def animated_splash(message_obj, u, user_db):
    coins        = user_db.get("coin") or 0
    total_images = user_db.get("number_create_image") or 0
    total_videos = user_db.get("number_create_video") or 0
    total_bypass = user_db.get("proxy") or 0
    package      = user_db.get("package") or "free"
    ...
    final_text = splash_final(u.first_name or "bạn", coins, total_images,
                              total_bypass, total_videos, package)
    try:
        await m.edit_text(final_text,
                          reply_markup=kb_main(coins, package),
...
async def cmd_start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    user_db = get_or_create_user(str(u.id), u.username or u.first_name or "")
    full_clear_session(u.id)
    await animated_splash(update.message, u, user_db)

async def cmd_approve(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    """Admin: /approve <user_id> <pkg_id>"""
    u = update.effective_user
    if u.id not in ADMIN_IDS:
        return
    args = ctx.args
    if len(args) < 2:
        await update.message.reply_text("Dùng: /approve <user_id> <pkg_id>\nVí dụ: /approve 123456789 coin_200")
        return
    target_uid = args[0]
    pkg_id     = args[1]
    await payment_handler.admin_approve(ctx.bot, u.id, target_uid, pkg_id)
...
async def btn(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
        ...
        if await check_join(ctx.bot, u.id):
            ...
            await q.edit_message_text(
                splash_final(u.first_name or "bạn", coins,
                             ...
                             user_db.get("proxy",0),
                             user_db.get("number_create_video",0), package),
                reply_markup=kb_main(coins, package), parse_mode="MarkdownV2")
        else:
            await q.answer("❌ Bạn chưa join channel!", show_alert=True)
        return
    ...
    if d == "home":
        full_clear_session(u.id)
        user_db = get_user_db(str(u.id)) or user_db
        coins   = user_db.get("coin") or 0
        package = user_db.get("package") or "free"
        await q.edit_message_text(
            splash_final(u.first_name or "bạn", coins,
                         ...
                         user_db.get("proxy",0),
                         user_db.get("number_create_video",0), package),
            reply_markup=kb_main(coins, package), parse_mode="MarkdownV2"); return
    ...
    # ── Stats ──
    if d == "stats":
        spent = (user_db.get("number_create_image",0)*COST_IMAGE +
                 user_db.get("number_create_video",0)*COST_VIDEO)
        await q.edit_message_text(
            f"📊 *CLOTHESBOT · THỐNG KÊ*\n\n"
            f"🆔 ID: `{u.id}`\n"
            f"🏆 Hạng: *{rank_badge(coins)}*\n\n"
            f"┌─ 💰 *COINS* ─────────────────┐\n"
            f"│  Hiện có:   `{coins} xu`\n"
            f"│  {coin_bar(coins)}\n"
            f"│  Đã tiêu:   `{spent} xu`\n"
            f"└──────────────────────────────┘\n\n"
            f"┌─ 👗 *HOẠT ĐỘNG* ─────────────┐\n"
            f"│  🎨 Ảnh đã tạo:   `{user_db.get('number_create_image',0)}` lần\n"
            f"│  🎬 Video đã tạo: `{user_db.get('number_create_video',0)}` lần\n"
            f"│  🔗 Link đã dùng: `{user_db.get('proxy',0)}` lần\n"
            f"└──────────────────────────────┘",
    ...
    if d == "key_enter":
        ...
        await q.edit_message_text(
            ...
            reply_markup=kb_cancel(), parse_mode="MarkdownV2"); return

    # ── Payment menu ──
    if d == "payment_menu":
        await payment_handler.handle_payment_menu(q, user_db); return

    if d == "pay_coins_menu":
        await payment_handler.handle_coin_menu(q); return

    if d == "pay_vip_menu":
        await payment_handler.handle_vip_menu(q); return

    if d.startswith("buy_"):
        pkg_id   = d[4:]
        username = u.username or u.first_name or str(u.id)
        await payment_handler.handle_buy(q, ctx, str(u.id), username, pkg_id); return

    if d == "paid_confirm":
        await payment_handler.handle_paid_confirm(q, str(u.id), u.username or str(u.id)); return
    ...
    if d == "img_start":
        ...
        await q.edit_message_text(
            f"🎨 ✨ *TẠO ẢNH*\n\n"
            f"```\n  💰 Số dư:   {coins} xu\n  💸 Chi phí: {COST_IMAGE} xu\n```\n\n"
            f"📸 *BƯỚC 1 / 2*\nGửi ảnh bạn muốn chỉnh sửa:",
            reply_markup=kb_cancel(), parse_mode="MarkdownV2"); return
    ...
    if d == "vid_start":
        ...
        sess.pop("video_photo_id", None); sess.pop("video_photo_bytes", None)
        await q.edit_message_text(
            f"🎬 ✨ *TẠO VIDEO*\n\n"
            f"```\n  💰 Số dư:   {coins} xu\n  💸 Chi phí: {COST_VIDEO} xu\n  🎞️  Output:  480p | 5 giây\n```\n\n"
            f"📸 *BƯỚC 1 / 2*\nGửi ảnh bạn muốn chuyển thành video:",
            reply_markup=kb_cancel(), parse_mode="MarkdownV2"); return
    ...
    if d == "vid_from_last_image":
        ...
        if not last_img:
            await q.edit_message_text("❌ Không tìm thấy ảnh vừa tạo\\.",
                reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("🎨 Tạo Ảnh", callback_data="img_start")]]),
                parse_mode="MarkdownV2"); return
        if coins < COST_VIDEO:
            await q.edit_message_text(
                f"⚠️ *KHÔNG ĐỦ XU\\!*\n\n```\n  Cần: {COST_VIDEO} xu\n  Có:  {coins} xu\n```",
...
async def handle_photo(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    u = update.effective_user
    if not await check_join(ctx.bot, u.id):
        await send_join_prompt(lambda text, **kw: update.message.reply_text(text, **kw)); return
    sess  = get_session(u.id)
...
async def handle_text(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if state == "key":
        ...
        if status == "valid":
            use_key(text)
            new_bal = add_coins(str(u.id), BYPASS_REWARD)
            increment_proxy_count(str(u.id))
            await update.message.reply_text(
                f"```\n{BANNER_SUCCESS}\n```\n\n"
                f"🎉 *NHẬN XU THÀNH CÔNG\\!*\n\n"
                f"```\n  ✅ Key hợp lệ\n  💎 Nhận được:  +{BYPASS_REWARD} xu\n  💰 Số dư mới:  {new_bal} xu\n```",
                reply_markup=kb_after_key(new_bal), parse_mode="MarkdownV2")
        elif status == "used":
            await update.message.reply_text(
    ...
    if state == "wait_prompt":
        ...
        prompt     = text
        if not photo_id:
            await update.message.reply_text("❌ Không tìm thấy ảnh\\. Vui lòng gửi lại\\!", parse_mode="MarkdownV2")
            clear_session(u.id); return

        user_db = get_user_db(str(u.id)) or {}
        ok_spend, new_bal = spend_coins(str(u.id), COST_IMAGE)
        if not ok_spend:
            await update.message.reply_text(
        ...
        # ── HÀNG CHỜ (chỉ free user) ──
        is_free = is_free_user(str(u.id))
        if is_free:
            queue_msg_sent = [False]
            async def queue_progress_cb(pos, wait_s):
                queue_msg_sent[0] = True
                try:
                    await msg.edit_text(
                        msg_queue_waiting(pos, wait_s),
                        parse_mode="MarkdownV2")
                except: pass

            # Đưa vào hàng chờ và đợi lên slot
            await enqueue(str(u.id), queue_progress_cb)

            # Khi lên slot, khôi phục loading UI
            try:
                await msg.edit_text(
                    render_log_step(0, 9, ["  ✅ Đến lượt bạn!", f"  📝 Prompt: {prompt[:35]}"], "~30-40s", tick=0),
                    parse_mode="Markdown")
            except: pass
        ...
        try:
            ...
            updater_task.cancel()
            add_coins(str(u.id), COST_IMAGE)   # Hoàn xu
            if is_free: await release(str(u.id))
            await msg.edit_text(
                f"```\n{BANNER_ERROR}\n```\n\n❌ *XỬ LÝ THẤT BẠI*\n\n"
        ...
        # Cập nhật DB
        increment_image_count(str(u.id))
        fresh_sess = get_session(u.id)
        fresh_sess["last_image_bytes"] = result_bytes
        fresh_sess["last_image_name"]  = _photo_name

        user_db_new = get_user_db(str(u.id)) or {}
        new_bal_actual = user_db_new.get("coin") or new_bal

        await update.message.reply_photo(
            photo=result_bytes,
            caption=(
                f"✨ *CLOTHESBOT · KẾT QUẢ TẠO ẢNH*\n\n"
                f"```\n  ✅ Xử lý thành công\n  📝 Prompt: {prompt[:40]}\n  💰 Còn lại: {new_bal_actual} xu\n  🏆 Hạng: {rank_badge(new_bal_actual)}\n```\n\n"
                f"👇 *Muốn tạo video từ ảnh này không?*"
            ),
            parse_mode="MarkdownV2", reply_markup=kb_after_image(new_bal_actual))
        await msg.delete()
        return
    ...
    if state == "wait_video_prompt":
        ...
        ok_spend, new_bal = spend_coins(str(u.id), COST_VIDEO)
        if not ok_spend:
            await update.message.reply_text(
        ...
        try:
            ...
            video_bytes, video_url = await loop.run_in_executor(
                None, pika_create_account_and_generate, _vbytes, _vname, prompt, log_cb_q)
        except Exception as e:
            updater_task.cancel()
            add_coins(str(u.id), COST_VIDEO)
        ...
        updater_task.cancel()
        increment_video_count(str(u.id))
        user_db_new = get_user_db(str(u.id)) or {}
        new_bal_actual = user_db_new.get("coin") or new_bal
        ...
        await update.message.reply_video(
            ...
            parse_mode="MarkdownV2", reply_markup=kb_after_video(new_bal_actual), supports_streaming=True)
        await msg.delete()
        return
